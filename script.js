const init = () => {
	...
	const careerGoalTemplate = document.querySelector('#career-goal-template')
		.text;
	const accomplishmentTemplate = document.querySelector(
		'#job-accomplishment-template',
	).text;
	...
	data.jobs.forEach(job => {
		...
		job.accomplishments.forEach(accomplishment => {
			accomplishments.push(
				accomplishmentTemplate.replace(
					/\{accomplishment\}/,
					accomplishment,
				),
			);
